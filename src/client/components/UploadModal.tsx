...
const UploadModal: React.FC<UploadModalProps> = (props) => {
  ...
  const predict = async () => {
    ...
    const nutritionixRes = await apiService("api/nutrients", "POST", {
      query: topPrediction.className,
    });

    const food = nutritionixRes.foods[0];
    setMeal({
      name: food.food_name,
  ...
  return (
    <div className="flex fixed w-screen h-screen justify-center items-center bg-black/50">
      <div className="bg-white rounded-lg px-6 py-5 w-full md:w-1/2 lg:w-1/3 mx-3">
